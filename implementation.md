...
The `core` crate will utilize Generational Arenas (e.g., the `slotmap` crate) for entity storage. This avoids Rust borrow checker conflicts and prevents use-after-free errors when entities (monsters, items) are created and destroyed during the simulation, without resorting to complex ECS frameworks.
...
## 4.3 Spatial Systems
Line of Sight (FOV) and Pathfinding (A* or Dijkstra) algorithms must be written strictly within `core` to facilitate the auto-explore loop without relying on external rendering libraries.
...
- Set up `slotmap` in `core` and `ChaCha8Rng`.
- Implement `RunState` and basic map structure.
- Build the minimal Macroquad `app` shell to render a simple grid concurrently, proving `core` and `app` communicate deterministically.
...
## Milestone 2 — Spatial Rules & Interrupt UI (10–12 hrs)
- Write FOV and Pathfinding in `core`.
...
- Ironman autosave (serialize `Game` state to disk).
